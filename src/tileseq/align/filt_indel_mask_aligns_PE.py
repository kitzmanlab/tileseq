...
def main():
    ...
    opts = argparse.ArgumentParser(description='filter out PE alignments with long indels or masked regions, or w/ supplementary alignments')
    ...
    for qn, _aligns in itertools.groupby( bamin, key=lambda ln:ln.query_name ):
        ...
        if ( len(aligns)!=2 ) or \
           not ( aligns[0].is_proper_pair and aligns[0].is_proper_pair ):
            n_filt_notproperpair += 1
        else:
            if aligns[0].is_unmapped or aligns[1].is_unmapped:
                n_filt_unaligned+=1
                continue

            cigs = aligns[0].cigartuples
            curmaxdel = max( [ 0 ] + [ t[1] for t in cigs if t[0]==2 ]  ) #2:Del
            cigs = aligns[1].cigartuples
            curmaxdel = max( [ curmaxdel ] + [ t[1] for t in cigs if t[0]==2 ]  ) #2:Del
            if curmaxdel>maxdel:
                n_filt_longdel+=1
                continue
            ...
            cigs = aligns[1].cigartuples
            curmaxins = max( [ curmaxins ] + [ t[1] for t in cigs if t[0]==1 ]  ) #1:Ins
            if curmaxins>maxins:
                n_filt_longins+=1
                continue
            ...
            cigs = aligns[1].cigartuples
            curmaxclip = max( [ curmaxclip ] + [ t[1] for t in cigs if t[0]==4 or t[0]==5 ]  ) #4/5: Soft/Hard Clip
    ...
    mout={
          'aligns_pass':n_pass,
          'aligns_fail_notproppair':n_filt_notproperpair,
